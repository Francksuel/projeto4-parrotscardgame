...
inicio()
function inicio(){
quant = Number (prompt("Quantas cartas quer jogar? (Número par de 4 a 14)"))
...
function fimdejogo (){
    ...
    let reinicio=prompt ("Quer jogar novamente? (sim/não)");
    while (reinicio!=="sim" && reinicio!=="não"){
        reinicio=prompt ("Responda sim ou não!");
    }
    if (reinicio=="sim"){
        quant=0
        selecionadas =[];
